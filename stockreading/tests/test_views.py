...
def test_view_create(client):
    resp = client.post(
        "/app/stockreading/",
        data={"ref_id": ref_id, "expiration_date": "2019-12-12T00:00"},
    )
...
def test_view_update(client, monkeypatch, stockreading):
    ...
    monkeypatch.setattr(
        StockReading, "get_from_id", MagicMock(return_value=stockreading)
    )
...
def test_synchronize(client, monkeypatch):
    ...
    resp = client.post(
        "/app/stockreading/synchronize/", data=data, headers={"content-type": "json"}
    )
    assert resp.status_code == 200
